/* Code taken from https://github.com/prakhar1989/progress-cpp with slight modifications by andrewmzhang*/
...
public:
    ...
    void display() const {
        ...
        float curr_progress = progress * 100.0;
        float eta = (1 - progress) / (progress / float(time_elapsed)) / 1000.0;

        std::cout << "] " << int(curr_progress) << "% "
                  << float(time_elapsed) / 1000.0 << " ETA: " << eta << "s\r";
        std::cout.flush();
    }
